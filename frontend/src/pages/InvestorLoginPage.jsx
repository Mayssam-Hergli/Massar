...
const VALID_EMAIL = 'investor@massar.tn'
...
export default function InvestorLoginPage() {
  ...
  const submit = (e) => {
    ...
    if (email.trim() === VALID_EMAIL && password === VALID_PASSWORD) {
      sessionStorage.setItem('investor_authed', '1')
      ...
      <div className="flex-1 flex items-center justify-center px-4 py-16">
        <div className="w-full max-w-md">
          <div className="text-center mb-8 flex flex-col items-center">
            <div className="w-12 h-12 rounded-full flex items-center justify-center mb-3" style={{ backgroundColor: '#EEF1FF' }}>
              <Shield size={22} color="#3346AC" />
            </div>
          ...
          <div className="bg-white rounded-2xl p-8" style={{ boxShadow: '0 8px 40px rgba(51,70,172,0.08), 0 1px 4px rgba(51,70,172,0.05)', border: '1px solid rgba(112,150,209,0.12)', borderTop: '3px solid #3346AC' }}>
            <form onSubmit={submit} className="flex flex-col gap-5">
              <div>
                <label className="block text-xs font-semibold mb-2" style={{ ...INTER, color: '#7096D1' }}>{isAr ? 'البريد الإلكتروني' : 'Email'}</label>
                <input type="email" value={email} onChange={(e) => setEmail(e.target.value)} onFocus={() => setEmailFocus(true)} onBlur={() => setEmailFocus(false)} required placeholder="investor@massar.tn" style={inputStyle(emailFocus)} />
              </div>
              <div>
              ...
              <button type="submit" className="w-full py-3 rounded-xl font-semibold text-white text-sm" style={{ ...INTER, backgroundColor: '#3346AC' }}>
